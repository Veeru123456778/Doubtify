...
const NotificationPopup = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-black bg-opacity-50 justify-end ">
      <div className="relative bg-white max-h-auto w-96  shadow-lg overflow-hidden  flex flex-col animate-slide-in">
        {/* mt-14 mr-[10%] */}
  ...
  <div className="space-y-4">
    {notifications.map((notification, index) => (
      <div key={index} className="flex items-start space-x-3 p-3 bg-white rounded-lg shadow-md">
        <img
          src={userImage}
          alt="User"
        ...
        <div className="flex flex-col">
          <p className="text-sm font-semibold">@{notification.name}           <span className="text-xs text-gray-700">{notification.action}</span>
 </p>
           <div className='flex justify-between'>
          <p className="text-xs text-gray-500">{getTimeString(notification.time)}</p>
          </div>
        </div>
      </div>
    ))}
